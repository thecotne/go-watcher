Watcher [![GoDoc](https://godoc.org/github.com/thecotne/go-watcher?status.svg)](https://godoc.org/github.com/thecotne/go-watcher) [![Build Status](https://travis-ci.org/thecotne/go-watcher.svg?branch=master)](https://travis-ci.org/thecotne/go-watcher)
=======
...
## Installation
  ...
  `go get github.com/thecotne/go-watcher`

  Install the binary under go/bin folder:

  `go install github.com/thecotne/go-watcher/cmd/watcher`
...
If you want to run Watcher in a containerized local environment, you can achieve this by using [thecotne/go-watcher](https://hub.docker.com/r/thecotne/go-watcher/) image in Docker Hub. There is an example project under [/docker-example](https://github.com/thecotne/go-watcher/tree/dockerfile-gvm/docker-examples) directoy. Let's try to dockerize this example code first.
...
In our example, we are creating a server that listens to port 7000 and responds to all clients with "watcher is running" string. The most essential thing to run your code in Docker is, mounting your project volume to a container. In the containerized Watcher, our GOPATH is set to /go directory by default, so you need to mount your project to this GOPATH.
  ...
  `docker run -v /path/to/hello:/go/src/hello -p 7000:7000 thecotne/go-watcher watcher -run hello`
...
Containerized Watcher also supports different versions of Go by leveraging [gvm](https://github.com/moovweb/gvm). Currently it only supports major versions right now. If you don't set anything, by default Watcher will pick version 1.7. If you want to change the Go version, you can use GO_VERSION environment variable. Currently it only supports 1.4, 1.5, 1.6, 1.7 at the moment
   ...
   `docker run -v /path/to/hello:/go/src/hello -e GO_VERSION=1.6 -p 7000:7000 thecotne/go-watcher watcher -run hello`
